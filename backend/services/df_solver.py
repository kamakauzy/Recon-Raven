...
def _bearing_line_distance(tx_lat: float, tx_lon: float,
...
def solve_triangulation(measurements: List[DFMeasurement]) -> Optional[DFEstimate]:
    ...
    lon0 += 500 * math.sin(math.radians(avg_bearing)) / (EARTH_RADIUS_M * math.cos(math.radians(lat0))) * (180 / math.pi)

    def cost(params):
        tx_lat, tx_lon = params
        total = 0
        for m in measurements:
            dist = _bearing_line_distance(tx_lat, tx_lon,
                                          m.latitude, m.longitude,
                                          m.bearing_deg)
            total += (dist * m.confidence) ** 2
        return total

    result = minimize(cost, [lat0, lon0], method='Nelder-Mead',
    ...
    logger.info("DF estimate: %.6f, %.6f (CEP %.0fm from %d bearings)",
                estimate.latitude, estimate.longitude,
                estimate.cep_m, estimate.num_bearings)
...
def compute_distance(lat1: float, lon1: float,
                     lat2: float, lon2: float) -> float:
