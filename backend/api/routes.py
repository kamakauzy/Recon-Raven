...
async def generate_report(db: AsyncSession = Depends(get_db)):
    ...
    bursts = [e for e in events if e.event_type == "burst"]
    alerts = [e for e in events if e.event_type == "alert"]

    if not bursts and not alerts:
    ...
    if bursts:
        ...
        lines.append("")
        lines.append("| Time | Freq (MHz) | Peak (dB) | Duration (ms) |")
        lines.append("|------|-----------|-----------|---------------|")
        for b in bursts[:50]:
            t = b.timestamp.strftime("%H:%M:%S") if b.timestamp else "?"
            f = f"{b.freq_mhz:.4f}" if b.freq_mhz else "?"
            p = f"{b.peak_power_db:.1f}" if b.peak_power_db else "?"
            d = f"{b.duration_ms:.0f}" if b.duration_ms else "?"
            lines.append(f"| {t} | {f} | {p} | {d} |")
        lines.append("")
        lines.append("---")
        lines.append("")
    ...
    if alerts:
        ...
        for a in alerts[:50]:
            ...
            f = f"{a.freq_mhz:.4f}" if a.freq_mhz else "?"
            p = f"{a.peak_power_db:.1f}" if a.peak_power_db else "?"
            lines.append(f"- **{t}** — {f} MHz, {p} dB")
        lines.append("")

    lines.append("---")
    lines.append("")
    lines.append("*Report generated by Recon-Raven intel_packager*")
    ...
    report = Report(
        ...
        title=f"Intel Summary {now.strftime('%Y-%m-%d %H:%M')}",
